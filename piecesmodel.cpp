...
QVariant PiecesModel::data(const QModelIndex &index, int role) const
{
    ...
    if (role == Qt::DecorationRole)
        ...
        return polygons.value(index.row());

    return QVariant();
}

void PiecesModel::addPiece(const QPixmap &pixmap, const QPoint &location, const QPolygon &polygon)
{
    int row;
...
QMimeData *PiecesModel::mimeData(const QModelIndexList &indexes) const
{
    ...
    foreach (QModelIndex index, indexes) {
        if (index.isValid()) {
            ...
            QPolygon polygon = qvariant_cast<QPolygon>(data(index, Qt::UserRole+2));
                               ...
                               int row, int column, const QModelIndex &parent)
{
    ...
    while (!stream.atEnd()) {
        ...
        polygons.insert(endRow, polygon);
        endInsertRows();
